...
namespace Backblaze_Client
{
    ...
    public class BackblazeClient
    {
        ...
        public BackblazeClient(BackblazeConfig config, Logger logger)
        {
            _config = config;
            _logger = logger;
        ...
        private async Task<(string, string)> GetUploadAuthorization()
        {
            ...
            return (jsonElement.GetProperty("uploadUrl").ToString(),jsonElement.GetProperty("authorizationToken").ToString());
        }
        ...
        public async Task<string> Upload(string fileName,byte[] fileContent)
        {
            ...
            _logger.Log(doc.ToString());
            _httpClient.DefaultRequestHeaders.Remove("Authorization");
            return doc.GetProperty("fileId").ToString();
        }
        ...
        public async Task<string> GetFileName(string fileId)
        {
            ...
            _logger.Log(doc.ToString());
            return doc.GetProperty("fileName").ToString();
        }
        ...
        public async Task Delete(string fileId, string fileName)
        {
            ...
            _logger.Log(await responseMessage.Content.ReadAsStringAsync());
        }
    }
}